...
function addNewEdField() {
  ...
  aqOb.insertBefore(newNode, document.getElementById("aqAddButton"));
  aqOb.insertBefore(removeButton, document.getElementById("aqAddButton"));
...
function removeLastEdField() {
  let aqOb = document.getElementById("ed");

  // Find the last textarea and remove it
  let lastTextarea = aqOb.querySelectorAll(".edField");
  if (lastTextarea.length > 0) {
    lastTextarea[lastTextarea.length - 1].remove();
  }

  // Find the last remove button and remove it
  let lastRemoveButton = aqOb.querySelectorAll(".btn-danger");
  if (lastRemoveButton.length > 0) {
    lastRemoveButton[lastRemoveButton.length - 1].remove();
  }
}
...
function generateCV() {
  ...
  for (let e of languages) {
    if (e.value === "") {
      return alert("Please fill the language field")
    }
  ...
  for (let e of educationF) {
    if (e.value === "") {
      return alert("Please fill the education field")
    }
  ...
  for (let e of wes) {
    if (e.value === "") {
      return alert("Please fill the experience field")
    }
  ...
  for (let e of aqs) {
    if (e.value === "") {
      return alert("Please fill the professional skills field")
    }
